...
def train_epoch(summary, summary_writer, cnn, model, loss_fn, optimizer,
    ...
    for step in range(steps):
        ...
        target_train = Variable(target_train.long().cuda(async=True))
        # print(data_train.shape,target_train)
        ...
        #     torch.cuda.FloatTensor).sum().data * 1.0 / probs.shape[0]
        # print(predicted,target_train)
        # print((predicted == target_train).sum())
        acc_data = (predicted == target_train).type(
            torch.cuda.FloatTensor).sum().data * 1.0  / predicted.shape[0]
        loss_data = loss.data
...
def valid_epoch(summary, model, loss_fn,
    ...
    with torch.no_grad():
        for step in range(steps):
            ...
            data_valid = Variable(data_valid.float().cuda(async=True))
            target_valid = Variable(target_valid.long().cuda(async=True))
            ...
            _, predicted = torch.max(output.data, 1)
            # probs = output.sigmoid()
            # print(float(probs.shape[0]))
            # predicts = (probs >= 0.5).type(torch.cuda.FloatTensor)
            # acc_data = (predicts == target_valid).type(
            #     torch.cuda.FloatTensor).sum().data * 1.0 / probs.shape[0]
            acc_data = (predicted == target_valid).type(
                torch.cuda.FloatTensor).sum().data * 1.0  / predicted.shape[0]
            loss_data = loss.data
